...
const DetalhesEmprestimo = ({ idEmprestimo }: DetalhesEmprestimoProps) => {
  ...
  useEffect(() => {
    const carregarDetalhesEmprestimo = async (id: string) => {
      try {
        const response = await fetch(`https://api.helenaramazzotte.online/api/emprestimos/${id}`);
  ...
  const devolverLivro = async () => {
    try {
      const response = await fetch(`https://api.helenaramazzotte.online/api/emprestimos/${idEmprestimo}/devolver`, {
        method: 'PUT',
      });
