...
export type Checkin = { id?: number; public_code: string; child_name_snapshot: string; guardian_name_snapshot: string; guardian_email_snapshot: string | null; guardian_phone_snapshot: string | null; child_birth_date_snapshot: string | null; child_notes_snapshot: string | null; room_name_snapshot: string; created_at: string };
...
async function request<T>(path: string, init: RequestInit = {}, authenticated = true): Promise<T> {
  ...
  const response = await fetch(`${API_BASE}/api${path}`, { ...init, headers, cache: "no-store" });
...
export const myKidsApi = {
  ...
  publicCheckin: (code: string) => request<Checkin>(`/public/mykids/checkins/${encodeURIComponent(code)}`, {}, false),
